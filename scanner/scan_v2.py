#!/usr/bin/env python3
""".bank.in Security Scorecard Scanner v2 - Threading-based, reliable"""
...
CREATE TABLE IF NOT EXISTS scans (
    ...
    http_status INTEGER, https BOOLEAN, tls_version VARCHAR,
    hsts BOOLEAN, hsts_preload BOOLEAN,
    x_frame_options VARCHAR, x_content_type VARCHAR, content_security BOOLEAN,
    server_header VARCHAR, title VARCHAR,
    dnssec BOOLEAN, dmarc_policy VARCHAR, spf_record VARCHAR, has_mx BOOLEAN,
    cpanel_detected BOOLEAN, admin_panel_detected BOOLEAN,
    git_exposed BOOLEAN, env_exposed BOOLEAN,
    ip_address VARCHAR, cloud_provider VARCHAR,
    score INTEGER, PRIMARY KEY (domain, scan_timestamp));
"""

RUBRIC = {
    "https": (0, 10), "tls_good": (0, 5), "hsts": (0, 7),
    "hsts_preload": (0, 3), "xfo": (0, 5), "xct": (0, 5), "csp": (0, 5),
    "dmarc_reject": (0, 12), "dmarc_quarantine": (0, 8), "spf": (0, 4), "mx": (0, 4),
    "dnssec": (0, 10),
    "no_git": (0, 5), "no_env": (0, 5), "no_phpinfo": (0, 5), "no_cpanel": (0, 5),
    "responds": (0, 5), "reachable": (0, 5),
}

def http_scan(domain):
    """Check HTTPS, headers, paths via curl"""
    result = {"https": False, "http_status": 0, "tls_version": "",
    ...
    try:
        ...
        cmd = ["curl", "-skIL", "--max-time", str(TIMEOUT),
               "-o", "/dev/null", "-w",
               "%{http_code}\\t%{ssl_verify_result}\\t%{content_type}\\t%{remote_ip}",
               f"https://{domain}"]
        ...
        # Full headers for security
        hdr = subprocess.run(["curl", "-skIL", "--max-time", str(TIMEOUT),
                              f"https://{domain}"],
                             capture_output=True, text=True, timeout=TIMEOUT+2).stdout.lower()
        result["hsts"] = "strict-transport-security" in hdr
        result["hsts_preload"] = "preload" in hdr
        
        for line in hdr.split("\n"):
            l = line.strip()
            if l.startswith("x-frame-options:"):
                result["x_frame_options"] = l.split(":", 1)[1].strip()
            elif l.startswith("x-content-type-options:"):
                result["x_content_type"] = l.split(":", 1)[1].strip()
            elif l.startswith("content-security-policy:"):
                result["content_security"] = True
            elif l.startswith("server:"):
                result["server_header"] = l.split(":", 1)[1].strip()
        
        # Title
        body = subprocess.run(["curl", "-skL", "--max-time", "8", f"https://{domain}"],
        ...
        # Path checks
        for path, key in [("/cpanel", "cpanel_detected"), ("/admin", "admin_panel_detected"),
                          ("/.git/config", "git_exposed"), ("/.env", "env_exposed"),
                          ("/phpinfo.php", "phpinfo_exposed")]:
            r = subprocess.run(["curl", "-sk", "--max-time", "4",
        ...
        if ip.startswith("3."): result["cloud_provider"] = "AWS"
        elif ip.startswith("13.") or ip.startswith("20.") or ip.startswith("40.") or ip.startswith("52."): result["cloud_provider"] = "Azure"
        elif ip.startswith("34.") or ip.startswith("35."): result["cloud_provider"] = "GCP"
        elif ip.startswith("104.16") or ip.startswith("104.2") or ip.startswith("172.64") or ip.startswith("172.65"): result["cloud_provider"] = "Cloudflare"
    except:
        pass
    return result

def dns_scan(domain):
    """DNS checks: DNSSEC, DMARC, SPF, MX"""
    r = {"dnssec": False, "dmarc_policy": "", "spf_record": "", "has_mx": False}
    try:
    ...
    try:
        ...
        for a in ans:
            ...
            if txt.startswith("v=DMARC1"):
                ...
                r["dmarc_policy"] = p.group(1) if p else txt[:60]
    except:
        pass
    try:
...
def compute_score(data):
    s = 0
    if data.get("https"): s += 10
    if data.get("tls_version", "").startswith("TLSv1.3"): s += 5
    elif data.get("tls_version", "").startswith("TLSv1.2"): s += 3
    if data.get("hsts"): s += 7
    if data.get("hsts_preload"): s += 3
    if data.get("x_frame_options"): s += 5
    if data.get("x_content_type") == "nosniff": s += 5
    if data.get("content_security"): s += 5
    dmarc = data.get("dmarc_policy", "")
    if dmarc == "reject": s += 12
    elif dmarc == "quarantine": s += 8
    elif dmarc: s += 4
    if data.get("spf_record"): s += 4
    if data.get("has_mx"): s += 4
    if data.get("dnssec"): s += 10
    if not data.get("git_exposed"): s += 5
    if not data.get("env_exposed"): s += 5
    if not data.get("phpinfo_exposed"): s += 5
    if not data.get("cpanel_detected"): s += 5
    if data.get("http_status", 0) in (200, 301, 302): s += 5
    if data.get("http_status", 0) not in (0, 404, 406): s += 5
    return min(s, 100)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
        ...
        for i, future in enumerate(as_completed(futures)):
            try:
                ...
                d = futures[future]
                batch_results.append({"domain": d, "score": 0, "scan_timestamp": datetime.now(timezone.utc),
                    "http_status": 0, "https": False, "tls_version": "", "hsts": False, "hsts_preload": False,
                    "x_frame_options": "", "x_content_type": "", "content_security": False, "server_header": "",
                    "title": "", "dnssec": False, "dmarc_policy": "", "spf_record": "", "has_mx": False,
                    "cpanel_detected": False, "admin_panel_detected": False, "git_exposed": False,
                    "env_exposed": False, "phpinfo_exposed": False, "ip_address": "", "cloud_provider": ""})
            ...
            if len(batch_results) >= 100:
                for r in batch_results:
                    try:
                        db.execute(insert_sql, (
                            ...
                            r.get("http_status", 0), r.get("https", False), r.get("tls_version", ""),
                            r.get("hsts", False), r.get("hsts_preload", False),
                            r.get("x_frame_options", ""), r.get("x_content_type", ""), r.get("content_security", False),
                            r.get("server_header", ""), r.get("title", ""),
                            r.get("dnssec", False), r.get("dmarc_policy", ""), r.get("spf_record", ""), r.get("has_mx", False),
                ...
                scores = [r.get("score", 0) for r in batch_results] if batch_results else [0]
                print(f"  [{scanned}/{total}] {rate:.0f} dom/s ETA {eta:.0f}s avg_score:{scores[0] if scores else 0}", flush=True)
    ...
    stats = db.execute("""
        ...
        FROM scans""").fetchone()
    print(f"  Total: {stats[0]} | Avg Score: {stats[1]} | HTTPS: {stats[2]} | HSTS: {stats[3]} | DNSSEC: {stats[4]} | DMARC=reject: {stats[5]}")
    
    db.close()

    # Export JSON for web API
    json_path = os.path.join(os.path.dirname(args.output), "scorecard.json")
    try:
        rows = db.execute("""
            SELECT s.*, d.bank_name, d.bank_type, d.dicgc_category
            FROM scans s
